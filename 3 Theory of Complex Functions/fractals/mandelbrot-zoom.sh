...
echo "Создание анимации зума Мандельброта..."
echo "Кадров: $FRAMES"
...
for ((n=0; n<FRAMES; n++)); do
    ...
    python 1_mandelbrot.py \
        --max_iter 100 \
        --x_min $CURRENT_X_MIN \
        --x_max $CURRENT_X_MAX \
        --y_min $CURRENT_Y_MIN \
        --y_max $CURRENT_Y_MAX \
        --output "temp/mandelbrot_zoom_$(printf "%03d" $n).png" > /dev/null 2>&1
done

echo ""
echo "Создание GIF анимации..."

# Создаем GIF с помощью ffmpeg (упрощенная версия)
ffmpeg -y \
    -framerate 15 \
    -i temp/mandelbrot_zoom_%03d.png \
    -vf "scale=800:-1:flags=lanczos" \
    -loop 0 \
    mandelbrot_zoom.gif

echo "Анимация готова: mandelbrot_zoom.gif"
